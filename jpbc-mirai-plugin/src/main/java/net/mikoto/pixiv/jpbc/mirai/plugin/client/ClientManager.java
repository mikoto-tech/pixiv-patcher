...
public class ClientManager {
     ...
     * @return Is success
     */
    private boolean removeClient(String key) {
    ...
    public boolean confirmClient(String key) {
        if (unverifiedClientMap.containsKey(key)) {
            normalClientMap.put(key, unverifiedClientMap.remove(key));
            return true;
        } else {
            return normalClientMap.containsKey(key);
        }
    }
     ...
     */
    public Client getUnverifiedClientByKey(String key) {
        return clientArrayList.get(unverifiedClientMap.get(key));
    }

    public String getNextConfirmTime() {
        synchronized (format) {
            return format.format(nextConfirmDate);
        }
    }

    protected void setNextConfirmTime(Date nextConfirmDate) {
        this.nextConfirmDate = nextConfirmDate;
    }
    ...
    protected void setClientNormal(Integer id) {
        if (unverifiedClientMap.containsValue(id)) {
            unverifiedClientMap.remove(clientArrayList.get(id).getClientKey());
        }
        for (String key :
                normalClientMap.keySet()) {
