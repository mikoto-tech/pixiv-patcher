...
public class ArtworkServiceImpl implements ArtworkService {
    ...
    public Artwork patchArtwork(int artworkId) throws GetArtworkInformationException, IOException, InvalidKeySpecException, NoSuchAlgorithmException, SignatureException, InvalidKeyException, WrongSignException, NoSuchMethodException, IllegalAccessException, GetImageException {
        ...
        if (artwork != null && MAIN_PROPERTIES.getProperty(IS_SAVE_ARTWORK).equals(TRUE)) {
            String dir = MAIN_PROPERTIES.getProperty(ARTWORK_PATH) + artwork.getAuthorName() + " - " + artwork.getArtworkTitle() + " - " + artwork.getArtworkId();
            createDir(dir);
            FileOutputStream fileOutputStream1 = new FileOutputStream(new File(dir + "/artworkData.json"));
            fileOutputStream1.write(artwork.toJsonObject().toJSONString().getBytes(StandardCharsets.UTF_8));
            ...
            for (String mode :
                ...
                for (int i = 0; i < times; i++) {
                    ...
                    fileOutputStream2.write(FORWARD_CONNECTOR.getImage(artwork.getIllustUrls().get(mode).replace(artwork.getArtworkId() + "_p0", artwork.getArtworkId() + "_p" + i)));
